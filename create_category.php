...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '') {
        ...
        try {
            $stmtCheck = $db->prepare("SELECT COUNT(*) FROM categories WHERE name = ?");
            ...
            if ($stmtCheck->fetchColumn() > 0) {
                ...
                log_activity($db, $user_id, 'Create Category', "Created asset category: $name");
                
                header('Location: org_setup.php?tab=tab-categories&msg=Category+created+successfully.&type=success');
                exit;
            }
        } catch (Exception $e) {
...
<section class="app-view">
    <div class="card-glow" style="max-width: 650px; margin: 40px auto; background: var(--bg-card); border: 1px solid var(--border-color); border-radius: var(--radius); padding: 32px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px; border-bottom:1px solid var(--border-color); padding-bottom:12px;">
            <h3 style="margin:0; font-family: var(--font-outfit); font-size:1.3rem;">Create Category</h3>
        ...
        <form action="create_category.php" method="POST">
            <div class="form-group" style="margin-bottom: 24px;">
                ...
                <input type="text" name="name" id="cat-name" required placeholder="e.g. Test Instruments" class="form-control" style="width:100%;">
            </div>
...
function addAttributeRow(name = '', type = 'text') {
    ...
    row.innerHTML = `
        <input type="text" name="attr_name[]" class="form-control" placeholder="Field Name (e.g. Screen Size)" value="${name}" required style="flex-grow:1;">
